Shader "Unlit/Frac_With_Gradient2_Shader"
{
    Properties
    {
        ...
        Pass
        {
            ...
            fixed4 frag (Interpolators i) : SV_Target
            {
               ...
               // b = frac(b);
                ...
                //return t;
                float OutputPattern = cos((i.uv.x + xOffset + xOffset2) * TAU * 5 + _Time.y) * 2 + 0.5;
                
                
                float4 A = lerp(_ColorA, _ColorB, OutputPattern);
                float4 B = lerp(_ColorC, _ColorD, OutputPattern);
                float4 OutputPatternColor = lerp(A, B, OutputPattern);
                //float output = lerp(_ColorA, _ColorB, OutputColor);
                return OutputPatternColor;
